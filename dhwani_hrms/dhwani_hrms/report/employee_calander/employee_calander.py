...
def execute(filters: Filters | None = None) -> tuple:
    ...
    # frappe.logger("utils").exception(f"Attendance Map: {attendance_map}")
...
def get_message() -> str:
    message = ""
    colors = ["green", "red", "orange", "green", "#318AD8", "", "", "green", "red"]
...
def get_attendance_map(filters: Filters) -> dict:
    ...
    for employee, leave_days in leave_map.items():
        for assigned_shift, days in leave_days.items():
            ...
            for day in days:
                frappe.logger("utils").exception(f"Day: {type(employee)}")
                for shift in attendance_map[employee].keys():
                    attendance_map[employee][shift][day] = "On Leave"
    today = getdate()

    for employee in attendance_map:
        if str(today.month) == filters.month and str(today.year) == filters.year:
            checked_in = frappe.db.get_all(
                "Employee Checkin",
                filters=[
            ...
            for shift in attendance_map[employee]:
                if checked_in:
                    attendance_map[employee][shift][today.day] = "Checked In"
                else:
                    attendance_map[employee][shift][today.day] = "Not In"
...
def get_attendance_records(filters: Filters) -> list[dict]:
    ...
    query = (
        ...
        .on(
            ...
            & (Attendance.docstatus == 1)  # Condition on Attendance for left join
            & (Attendance.company.isin(filters.companies))
            & (Extract("month", Attendance.attendance_date) == filters.month)
            & (Extract("year", Attendance.attendance_date) == filters.year)
        )
        .select(
            Employee.name.as_("employee"),  # Select from Employee table
            Employee.employee_name,  # Include employee name
            Extract("day", Attendance.attendance_date).as_("day_of_month"),
            Attendance.status,  # These might be None if no attendance record
            Attendance.shift,  # These might be None if no attendance record
        )
        .where(Employee.company.isin(filters.companies))
        .where(Employee.name.notin([1, 2]))  # Exclude those users
    )

    if filters.employee:  # If specific employee filter is active
        query = query.where(Employee.name == filters.employee)

    query = query.orderby(
        Employee.name, Attendance.attendance_date
...
def get_employee_related_details(filters: Filters) -> tuple[dict, list]:
    ...
    if group_by:
        ...
        for parameter, employees in groupby(
            sorted(employee_details, key=group_key), key=group_key
        ):
...
def get_attendance_summary_and_days(
    ...
    leave_case = (
        frappe.qb.terms.Case().when(Attendance.status == "On Leave", 1).else_(0)
    )
    sum_leave = Sum(leave_case).as_("total_leaves")

    half_day_case = (
        frappe.qb.terms.Case().when(Attendance.status == "Half Day", 0.5).else_(0)
    )
...
def get_leave_summary(employee: str, filters: Filters) -> dict[str, float]:
    ...
    Attendance = frappe.qb.DocType("Attendance")
    day_case = (
        frappe.qb.terms.Case().when(Attendance.status == "Half Day", 0.5).else_(1)
    )
        ...
        for __, attendance_dict in attendance_map.items():
            for __, attendance in attendance_dict.items():
                ...
                if attendance_on_day == "On Leave":
                    ...
                    break
                elif attendance_on_day in ["Absent", "Not In"]:
                    total_absent_on_day += 1
                elif attendance_on_day in ["Present", "Work From Home", "Checked In"]:
